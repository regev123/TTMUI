...
const executeSSHCommand = (conn, command, stdout, res) => {
  conn.exec(command, (err, stream) => {
    ...
    stream.on('close', () => {
      ...
      else res.status(400).json('Deployer Failed');
    });
...
router.post('/runDeployer', async (req, res) => {
  ...
  try {
    ...
    const ttmDeployerCommand = config.get('TTMDeployerCommand');
    ...
    conn.on('ready', () => {
      ...
      const command = `cd ${ttmHome}WORK_DIR/ && ${execClientsString} && cd ${ttmHome}deployer/ && ${ttmDeployerCommand}`;
